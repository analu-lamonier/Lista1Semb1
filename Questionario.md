...
Quando ocorre uma interrupção o valor salvo no LR que diz o que deve ser feito no retorno de uma interrupção, valor esse chamado de EXC_RETURN, tendo uma codificação especificando qual modo (handler ou thread) e qual stack (Process stack ou Main Stack) deverá ser retornado.Assim, o retorno de uma instrução é feito sobrescrevendo o PC e com o endereço de retorno, determinando o SP após decodificação do EXC_RETURN.
